...
namespace SQLDALHelper
{
    ...
    public class OracleHelper
    {
        ...
        /// 分页
        /// </summary>
        /// <param name="strOracle">SQL语句</param>
        /// <param name="page">当前页</param>
        /// <param name="PageSize">每页显示条数</param>
        /// <returns></returns>
        public static DataSet GetDataSet(string strOracle, int page, int PageSize = 10)
        {
            if (page <= 0)
                page = 1;
            var SQLStr = $@"SELECT * FROM(SELECT t.*, ROWNUM AS row_no FROM({strOracle}) t WHERE ROWNUM <= :V_MaxPage) tabPage WHERE tabPage.row_no >= :V_MinPage";
            int MaxPage = PageSize * page;
            int Minpage = PageSize * (page - 1);
            OracleParameter[] ArrParam = new OracleParameter[] {
                new OracleParameter("V_MaxPage", MaxPage),
                new OracleParameter("V_MinPage", Minpage),
            };
            return GetDataSet(CommandType.Text, SQLStr, ArrParam);
        }
        ...
        /// <param name="PageSize">每页显示条数</param>
        /// <returns></returns>
        public static DataSet GetDataSet(String StrSQl, string Order, int page, int PageSize = 10)
        {
            if (page <= 0)
                page = 1;
            var SQLStr = $@"select * from(select *,ROW_NUMBER() OVER(Order by { Order }) AS Row_Id from ({ StrSQl }) a ) b where Row_Id between :V_MinPage and :V_MaxPage";
            int MaxPage = PageSize * (page - 1);
            int Minpage = PageSize * page;
            OracleParameter[] ArrParam = new OracleParameter[] {
                new OracleParameter("V_MaxPage", MaxPage),
                new OracleParameter("V_MinPage", Minpage),
            };
        ...
        public static int ExecuteSqlTran(List<String> SQLStringList, params OracleParameter[] cmdParms)
        {
            int ret = 0;
            using (OracleConnection OracleConn = new OracleConnection(CONN_STRING_NON_DTC))
            {
