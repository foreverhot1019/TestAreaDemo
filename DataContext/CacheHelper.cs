...
namespace DataContext
{
    ...
    public class CacheHelper : GetKeyPerpoty
    {
        ...
        //默认连接字符串
        public static string ConnStr = ConfigurationManager.ConnectionStrings["DefaultConnection"]==null?"": ConfigurationManager.ConnectionStrings["DefaultConnection"].ToString();
        //特殊的缓存键值
        ...
        public static object Get_SetCache(Common.CacheNameS OCacheKey)
        {
            ...
            DataContext dbContext = new DataContext(ConnStr);
        ...
        /// 获取 缓存
        /// </summary>
        /// <param name="type">缓存枚举</param>
        /// <returns></returns>
        public static object Get_SetPropertyCache(Type type)
        {
            if (type == null)
                return null;
            string CacheName = "TypePropty_" + type.FullName;
            IEnumerable<PropertyInfo> ArrProperty = null;
            try
            {
                var Cacheobj = GetCache(CacheName);
                if (Cacheobj == null)
                {
                    ArrProperty = type.GetProperties(System.Reflection.BindingFlags.Instance |
                                    System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic).ToList();
                    lock (Common.lockCacheHelper)
                    {
                        SetCache(CacheName, ArrProperty);
                    }
                }
                else
                    ArrProperty = (IEnumerable<PropertyInfo>)Cacheobj;
            }
            catch (Exception ex)
            {
                string ErrMsg = CacheName + ":" + Common.GetExceptionMsg(ex);
        ...
        /// <param name="logPath"></param>
        /// <returns></returns>
        public static object Get_SetdbContextMember(DataContext AppdbContext = null, string logPath = "", string CacheName = "dbContextMember")
        {
            MemberInfo[] ArrMember = new MemberInfo[] { };
            try
            {
                if (AppdbContext == null) 
                    AppdbContext = new DataContext(ConnStr);
                if (string.IsNullOrEmpty(logPath))
                    logPath = LogFolder;
        ...
        /// <param name="logPath"></param>
        /// <returns></returns>
        public static object Get_SetApplicationRole(DataContextAppUser AppdbContext = null, string logPath = "", string CacheName = "ApplicationRole")
        {
            //缓存菜单
            ...
            {
                if (AppdbContext == null)
                    AppdbContext = new DataContextAppUser(ConnStr);
                if (string.IsNullOrEmpty(logPath))
                    logPath = LogFolder;
        ...
        /// <param name="logPath"></param>
        /// <returns></returns>
        public static object Get_SetApplicationUser(DataContextAppUser AppdbContext = null, string logPath = "", string CacheName = "ApplicationUser")
        {
            //缓存菜单
