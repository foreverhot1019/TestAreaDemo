...
namespace DataContext.Extensions
{
    public static class XMLHelper
    {
        ...
        private static bool AsyncWriteLog
        {
            ...
            {
                bool? _AsyncWriteLog;
                var CacheName = "AsyncWriteLog";
                var CacheObj = HttpRuntime.Cache[CacheName];
                if (CacheObj == null)
                {
                    ...
                    HttpRuntime.Cache.Insert(CacheName, IsWriteDataToLunece);
                    _AsyncWriteLog = IsWriteDataToLunece;
                }
                else
                    _AsyncWriteLog = (bool)CacheObj;
        ...
        public static string toXMLStr<T>(this T ObjVal, Encoding OEncoding = null, string Namespaces = "") where T : class, new()
        {
            ...
            try
            {
                ...
                using (XmlWriter writer = XmlWriter.Create(sb, new XmlWriterSettings { OmitXmlDeclaration = false, Encoding = OEncoding }))
                {
