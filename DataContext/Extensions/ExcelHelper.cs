    ...
    public class ExcelHelper
    {
        ...
        public static DataTable ToDataTable<T>(List<T> data)
        {
            ...
            for (int i = 0; i < props.Count; i++)
            {
                ...
                string displayName = Common.GetDisplayName(type, prop.Name);
        ...
        public static Stream ExportExcel<T>(Type modelType, List<T> list, params string[] IgnoredColumns)
        {
            ...
            using (ExcelPackage pck = new ExcelPackage())
            {
                ...
                for (int i = 1; i <= PropertyInfos.Count(); i++)
                {
                    ...
                    string displayName = Common.GetDisplayName(modelType, PropertyInfos[i - 1].Name);
        ...
        {
            //项目顶层命名空间
            string Top_NameSpace = System.Configuration.ConfigurationManager.AppSettings["Top_NameSpace"] ?? "KSWECDS";
            //网站顶层命名空间
            string WebTop_NameSpace = System.Configuration.ConfigurationManager.AppSettings["WebTop_NameSpace"] ?? "KSWECDS.Web";
            //项目类命名空间
            ...
            string ModelsNameSpace = (string.IsNullOrEmpty(WebTop_NameSpace) || string.IsNullOrEmpty(_ModelsNameSpace)) ? "KSWECDS.Web.Models" : WebTop_NameSpace + "." + _ModelsNameSpace;
            string ModelsTopNameSpace = string.IsNullOrEmpty(Top_NameSpace) ? "KSWECDS." : Top_NameSpace + ".";
        ...
        public static byte[] OutPutExcelByListModel<T>(T ObjT, string ExcelModelPath, int HeadLineRowIndex, int HeadLineCellsIndex, out string FilePath, string FileStartName)//List<obj>
        {
            ...
            //项目顶层命名空间
            string Top_NameSpace = System.Configuration.ConfigurationManager.AppSettings["Top_NameSpace"] == null ? "KSWECDS" : System.Configuration.ConfigurationManager.AppSettings["Top_NameSpace"].ToString();
            //网站顶层命名空间
            string WebTop_NameSpace = System.Configuration.ConfigurationManager.AppSettings["WebTop_NameSpace"] == null ? "KSWECDS.Web" : System.Configuration.ConfigurationManager.AppSettings["WebTop_NameSpace"].ToString();
            //项目类命名空间
            ...
            string ModelsNameSpace = WebTop_NameSpace + "." + Models_NameSpace;//"KSWECDS.Web.Models"
            string ModelsTopNameSpace = Top_NameSpace + ".";//"KSWECDS."
            ...
            List<TotalColumn> ArrColumnTotal = new List<TotalColumn>();
                                ...
                                if (retVal != null)
                                {
                                    switch (DataType.ToLower())
                                    {
                                        case "int":
                                            ...
                                            {
                                                TotalColumn Temp = new TotalColumn();
                                        ...
                                        case "int32":
                                            ...
                                            {
                                                TotalColumn Temp32 = new TotalColumn();
                                        ...
                                        case "int64":
                                            ...
                                            {
                                                TotalColumn Temp64 = new TotalColumn();
                                        ...
                                        case "decimal":
                                            ...
                                            {
                                                TotalColumn Tempdecima = new TotalColumn();
                                        ...
                                        case "double":
                                            ...
                                            {
                                                TotalColumn Tempdouble = new TotalColumn();
                                        ...
                                        case "float":
                                            ...
                                            {
                                                TotalColumn Tempfloat = new TotalColumn();
        ...
        public static byte[] OutPutExcelByListModel<T>(T ObjT, bool MoreSheet, string ExcelModelPath, int HeadLineRowIndex, int HeadLineCellsIndex, out string FilePath, string FileStartName)//List<obj>
        {
            ...
            if (ExcelFile.Exists)
            {
                try
                {
                    ...
                    {
                        ArrColumnTotal = new List<TotalColumn>();
        ...
        public static byte[] OutPutExcelByListModel<T>(Object ObjT) where T : class,new()
        {
            ...
            try
            {
                ...
                foreach (PropertyInfo pi in basePi)
                {
                    ...
                    if (WhereGetPi.Any())
                    {
                        ColumnName = Common.GetMetaDataDisplayName(GetType, pi.Name);
                    }
                    else
                    {
                        ...
                        if (WhereGetPi.Any())
                        {
                            ColumnName = Common.GetMetaDataDisplayName(GetType, pi.Name.Replace("NAME", "")) + "名称";
                        }
                    }
                    //设置单元格 名称
        ...
        /// <param name="ms"></param>
        /// <returns></returns>
        public static MemoryStream ConvertToPdf(Stream ms)
        {
            ...
            try
            {
                ...
                workbook.Save(OutMs, SaveFormat.Pdf);
                //var sopts = new SaveOptions();
                //sopts.ClearData = true;
                //sopts.SaveFormat = SaveFormat.Pdf;
                //workbook.Save(OutMs, sopts);
            }
            catch (Exception ex)
            {
        ...
        /// <param name="ms"></param>
        /// <returns></returns>
        public static MemoryStream ConvertToPdf(byte[] buffer)
        {
            var OutMs = new MemoryStream();
            try
            {
        ...
        /// <param name="ms"></param>
        /// <returns></returns>
        public static MemoryStream ConvertToPdf(Stream ms, bool t = true)
        {
            var OutMs = new MemoryStream();
            if (!t)
                return ConvertToPdf(ms);
            
            var DllPath = HttpContext.Current.Server.MapPath("/Aspose/Aspose.Cells.v18.1.0/Aspose.Cells.dll");
            var CellsAssm = Assembly.LoadFile(DllPath);
            // 获取该dll中命名空间 类
            Type TLicense = CellsAssm.GetType("Aspose.Cells.License");
            // 实例化该类
            object OLicense = Activator.CreateInstance(TLicense);
            // 获取该类中的方法SetLicense
            MethodInfo mSetLicense = TLicense.GetMethod("SetLicense", new Type[] { typeof(string) });
            string path = HttpContext.Current.Server.MapPath("/Aspose/License.lic");
            // 使用方法
            mSetLicense.Invoke(OLicense, new object[] { path });


            Type Tworkbook = CellsAssm.GetType("Aspose.Cells.Workbook");

            Type TSaveFormat = CellsAssm.GetType("Aspose.Cells.SaveFormat");
            //object OSaveFormat = Activator.CreateInstance(TSaveFormat);
            //MethodInfo mParse = TSaveFormat.GetMethod("Parse", new Type[] { typeof(Type), typeof(string) });
            var OSaveFormat = Enum.Parse(TSaveFormat,"Pdf",true);

            object Oworkbook = Activator.CreateInstance(Tworkbook, new object[] { ms });
            MethodInfo mSave = Tworkbook.GetMethod("Save", new Type[] { typeof(MemoryStream), TSaveFormat });
            mSave.Invoke(Oworkbook, new object[] { OutMs, OSaveFormat });
